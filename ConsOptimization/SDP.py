...
def Obj_2nd(m):
    return sum(m.pi[s]*sum(m.p2s[s,t]*(m.P_EV[s,t]) for t in m.T2) for s in S)
...
def ModelSetUp_2nd(p2s,S,T1,T2,pi,X_hat):
    # Instance
    m = pyo.ConcreteModel()
    # Define sets
...
def SDP():
    ...
    states_jump = 5 # 5 or 25 depending on wether we want 11 or 3 cuts
    ...
    for X_hat in List_charged:
        m_2nd = ModelSetUp_2nd(p2s,S,T1,T2,pi,X_hat)
        Solve(m_2nd)
    ...
    m_2nd = ModelSetUp_2nd(p2s,S,T1,T2,pi,FirstStageEnergy)
    Solve(m_2nd)
